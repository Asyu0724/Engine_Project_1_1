...
// 어떤 연출을 할지 선택하는 스위치
public enum EndEmotionAnim { None, Shake, Jump, Pop }

// 누가 말하는지 구분하기 위한 enum
public enum EndSpeakerPosition { Left, Right }

// 인스펙터에서 대화 데이터를 쉽게 넣기 위한 클래스
[System.Serializable]
public class EndDialogueLine
{
...
public class FinalDialogue : MonoBehaviour
{
    ...
    [Header("대화 데이터")]
    public DialogueLine[] fianlDialogues;
    
    private int currentFinalDialogueLineIndex = 0;
    ...
    void Update()
    {
        ...
        if (dialoguePanel.activeSelf && Input.GetKeyDown(KeyCode.Z))
        {
            if (isTyping)
            {
                ...
                StopAllCoroutines();
                dialogueText.text = fianlDialogues[currentFinalDialogueLineIndex].dialogueText;
                isTyping = false;
            }
            else
            {
    ...
    public void StartDialogue()
    {
        ...
        currentFinalDialogueLineIndex = 0;
        ShowDialogue();
    }

    private void ShowDialogue()
    {
        DialogueLine line = fianlDialogues[currentFinalDialogueLineIndex];
    ...
    {
        currentFinalDialogueLineIndex++;
        if (currentFinalDialogueLineIndex < fianlDialogues.Length)
        {
            ShowDialogue();
    ...
    {
        dialoguePanel.SetActive(false);
        BossDieEffect.instance.PlayDeathEffect();
    }
}